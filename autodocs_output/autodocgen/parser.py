...
def parse_function(node, file_path, use_ai=False, context=''):
    """Parses a node based on the provided context and file path, 
with optional AI-assisted parsing.

Args:
    node: The node to be parsed.
    file_path: The path to the file associated with the node.
    use_ai: A flag indicating whether to use AI-assisted parsing.
...
def parse_class(node, file_path, use_ai=False, context=''):
    """Parses a class from the given node and file path, utilizing AI assistance if specified. 
  Args:
    node: The node to parse.
    file_path: The path to the file being parsed.
    use_ai: A flag indicating whether to use AI assistance.
    context: The context in which the parsing is taking place. 
  Returns: 
    None"""
    methods = []
...
def parse_file(file_path, use_ai=False):
    """Parses a file and generates a summary.
 ...
 Raises:
   Exception: If AI summary fails and use_ai is True."""
    try:
...
def parse_python_files(directory, use_ai=False, ignore=[]):
    """Parses Python files in the specified directory, utilizing AI if requested, while ignoring specified files or directories.
...
def group_functions_by_file(functions):
    """Groups a list of functions by their defining file.

Args:
    functions: A list of functions to be grouped by file.

Returns:
    A dictionary mapping file names to lists of functions defined in each file."""
    grouped = defaultdict(list)
